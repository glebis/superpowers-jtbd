---
name: jtbd
description: "Use when articulating what a project does, who it's for, and why it matters. Captures jobs, switch forces, and outcomes through structured interview. Feeds into brainstorming via jtbd.json handoff."
---
...
6. Write four files using templates:
...
7. If the current working directory is a git repo, create a symlink: `./jtbd.json → ~/jtbd/<slug>/jtbd.json`. If a symlink already exists, overwrite it.
8. Report all paths.

## Downstream pipeline (superpowers integration)

The `jtbd.json` is a contract between this skill and downstream superpowers agents. See `references/superpowers_handoff.md` for the full field mapping.

**Short version:** when brainstorming starts and a `jtbd.json` exists, the `superpowers-jtbd` PreToolUse hook auto-injects it as context. Brainstorming then skips questions the JSON already answers (who, what, why, constraints) and focuses on what it doesn't (how, architecture, scope, technical choices). Switch forces inform approach selection. Open questions become brainstorming priorities.

**Chain:** `/superpowers-jtbd:jtbd` → `jtbd.json` → `/superpowers:brainstorming` (auto-bridged by hook) → writing-plans → implementation

After the interview, suggest: "Ready to brainstorm approaches? Run `/superpowers:brainstorming` — your JTBD context will be picked up automatically."
